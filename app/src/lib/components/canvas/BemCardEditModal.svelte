<script lang="ts">
	/**
	 * BemCardEditModal — single source of truth for the BEM rich-card edit
	 * modal. Used by both the Matrix tab (concept/domain header) and the
	 * Canvas tab (card click). Handles all three BEM card types with the
	 * fields that BEM tracks:
	 *
	 *   - Domain   : name, description, aliases, owner, notes
	 *   - Concept  : name, description, aliases, w-type, definition (category +
	 *                differentiator with @-mention autocomplete), notes
	 ...
	 *
	 * Why not the shared CardEditModal? CardEditModal handles only
	 * name+description+optional fields. The BEM card edit needs domain-owner,
	 * w-type and the rich definition builder — all BEM-specific. Keeping it
	 * in apps/business-event-matrix/ rather than packages/shared/ matches the
	 * Core Model vs Language rule: shared owns the cross-canvas core; this
	 * is a BEM-specific surface.
	 */
	...
	// Detect type from labels. Fallback to 'event' so the modal renders
	// something sensible for unknown labels.
	type CardType = 'domain' | 'concept' | 'event';
	const cardType: CardType = (() => {
		const labels = getNodeLabels(node);
		if (labels.includes('bem_domain')) return 'domain';
		if (labels.includes('global_concept') || labels.includes('bem_dimension')) return 'concept';
		return 'event';
	})();

	const typeLabel =
		cardType === 'domain' ? 'Domain' : cardType === 'concept' ? 'Concept' : 'Core Business Event';
