...
The Business Event Matrix is a **local-only dev tool**. It is designed to run on your own machine, alongside Claude Code or another local AI assistant. It is NOT designed for:
...
The dev server binds to `127.0.0.1:5121` and the SvelteKit `+server.ts` API routes write JSON files directly to `data/`. There is no authentication. Anyone who can reach the port can read, overwrite, or delete your matrices.

If you need to share a matrix, use the **Export JSON / PDF / CSV / Excel** buttons and share the downloaded files — never the server.
...
- **`data/*.json` file format.** Anyone with shell access can hand-edit the JSON files. That's by design — it's also how the included Claude skill reads your matrices.
...
- We don't ship the abandoned `xlsx` package. Writing XLSX is done via `exceljs`; reading XLSX is intentionally unsupported and a CI script (`scripts/check-no-xlsx-read.sh`) blocks it from being reintroduced.
